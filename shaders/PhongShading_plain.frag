...
File Name: PhongShading_plain.frag
...
vec3 CalcDirLight( PointLight light, vec3 normal, vec3 viewDir )
{
    ...
    // Combine results
    vec3 ambient = light.ambient * reflectDir ;
    vec3 diffuse = light.diffuse * diff ;
    vec3 specular = light.specular * spec ;
    
    return globalAmbient + ( ambient + diffuse + specular );
}
...
vec3 CalcSpotLight( PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir )
{
    ...
    vec3 result = material.emissive + globalAmbient + Attenuation * spotLightEffect* (diffuse+specular + ambient);
    return result;
